...
function DOCOrderDialog_View(id,options){
	...
	*/
	...
	//Объем
	sub_cont.addElement(new Control(uuid(),"span",{"value":"Транспортировочный объем: ", visible: false}));
	this.m_totVol = new Control(uuid(),"span", {visible:false});
	sub_cont.addElement(this.m_totVol);
	sub_cont.addElement(new Control(uuid(),"span",{"value":" м3", visible: false}));
	//
	//масса
	sub_cont.addElement(new Control("prod_tot_w","span",{"value":", масса груза: ", visible: false}));
	this.m_totWt = new Control("prod_tot_wt","span", {visible:false});
	sub_cont.addElement(this.m_totWt);
	sub_cont.addElement(new Control("prod_tot_wt2","span",{"value":" т.", visible: false}));
	...
	// 	}		
	// });
	this.m_delivAddress = new EditString(id+"_deliv_destination",
	{"labelCaption":"Адрес доставки:",
			"name":"deliv_destination",
			"tableLayout":false,
			"buttonClear":false,
			"attrs":{"maxlength":500}//,"size":20
	     }
		);
	this.bindControl(this.m_delivAddress,
		{"modelId":model_id,"valueFieldId":"deliv_destination","keyFieldIds":null},
		{"valueFieldId":"deliv_destination","keyFieldIds":null}
	);	
	cont.addElement(this.m_delivAddress);		
