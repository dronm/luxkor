...
DOCOrderDOCTProductDialog_View.prototype.onProductSelected = function(productId){
	...
	*/
	...
	if (productId!="undefined"){
		console.log("getting product object")
		//get product attrs
		var self = this;
...
DOCOrderDOCTProductDialog_View.prototype.onGetProductAttrs = function(model){	
	...
	var cont =new ControlContainer("quant_cont","div",{"className": "form-group"});//get_bs_col()+"4"
	...
	this.m_quantCtrl = new DOCOrderQuantEdit(id+"_quant",
		...
		"events":{"input":function(){
					self.m_totQuantCtrl.setValue( self.m_quantCtrl.getValue() );
					self.calcTotalSum();
				}
			}
		}		
	...
	var cl_lbl = "control-label "+get_bs_col()+"4";
	var cl_fl = get_bs_col()+"6 DOCOrderTot";
...
DOCOrderDOCTProductDialog_View.prototype.calcTotalSum = function(){
	const price = this.m_totPriceCtrl.getValue();
	const q = this.m_quantCtrl.getValue();
	var tot = toFloat(price)*toFloat(q);
	//can be round request
	if(this.m_priceRoundCtrl.getValue()=="true"){
		tot = Math.ceil(tot);
	}
	this.m_totSumCtrl.setValue(tot.toFixed(2));
	this.m_priceEditted = true;
}
